...
class ShowContentView(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        self.path = Input(
            placeholder='Path do Diretório', type='text', id='input'
        )
        yield self.path
        ...
        with Container(id='cont02'):
            ...
            self.language = Select(
                ((lang, lang) for lang in Languages), id='select_lang'
            )
            yield self.language

        with Horizontal(id='cont03'):
            self.button_extrair = Button(
                'Extrair Texto', variant='success', id='b_extrair'
            )
            yield self.button_extrair
            self.button_exibir = Button(
                'Exibir Imagem', variant='warning', id='b_exibir'
            )
    ...
    def on_button_pressed(self, event):
        self.content = self.controller.extract_text(
            self.file_path, self.language.value
        )
